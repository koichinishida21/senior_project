...
echo "</center>";
  ...
  && (!empty($_POST['state'])) && (!(trim($_POST['state']) == ""))){
		...
		$query3 = "UPDATE Committee SET committeeUser = ?, lastName = ?, firstName = ?, address = ?, city = ? , stateNum = ?,
    phone = ?, email = ?, churchID = ?, admin = ? WHERE committeeID = ?;";
   	$stmt3 = $mysqli->prepare($query3);
		$stmt3 -> execute([$_POST['user'], $_POST['lname'], $_POST['fname'],
    ...
    $_POST['cphone'], $_POST['cemail'], $_POST['church'], $_POST['admin'], $_POST['cid']]);
		...
		if($stmt3) {
			...
			$_SESSION["message"] = "ERROR!! Could not update the Committee";
			redirect("comm.php");
		}
		redirect("comm.php");

	}else{
		$_SESSION["message"] = "Unable to update a Committee! Fill in all information!";
		redirect("comm.php");
	}
	...
	redirect("comm.php");
      ...
      echo "<form action='comm_u.php' method='POST'>\n";
      echo "<div class='row'>\n";
