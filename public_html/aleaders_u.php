...
echo "</center>";
    ...
    && (!empty($_POST['user'])) && (!(trim($_POST['user']) == ""))
  ...
  && (!empty($_POST['state'])) && (!(trim($_POST['state']) == ""))){
    ...
    $stmt-> execute([$_POST['user']]);
          ...
          if ($rowNum = $stmt -> rowCount() > 1) {
            ...
            redirect("aleaders_u.php");

          }else{

        		$query3 = "UPDATE Leader SET leaderUser = ?, lastName = ?, firstName = ?, gender = ?, pos = ?, address = ?, city = ? , stateNum = ?,
            phone = ?, email = ?, parentLname = ?, parentFname = ?,
            parentPhone = ?, parentEmail = ?, schoolID = ?, churchID = ?, admin = ? WHERE leaderID = ?;";
           	$stmt3 = $mysqli->prepare($query3);
        		$stmt3 -> execute([$_POST['user'],$_POST['lname'], $_POST['fname'], $_POST['gender'], $_POST['pos'],
      ...
      echo "<form action='aleaders_u.php' method='POST'>\n";
      echo "<div class='row'>\n";
      ...
      echo "<p style='color: black'>Username*: <input type=text name='user' value = '".$row['leaderUser']."'></p>\n";
